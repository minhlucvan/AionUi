...
const MezonConfigForm: React.FC<MezonConfigFormProps> = ({ pluginStatus, modelList, selectedModel, onStatusChange, onModelChange }) => {
  const { t } = useTranslation();

  // Mezon credentials
  const [mezonToken, setMezonToken] = useState('');
  const [botId, setBotId] = useState('');

  const [testLoading, setTestLoading] = useState(false);
  const [credentialsTested, setCredentialsTested] = useState(false);
  const [touched, setTouched] = useState({ token: false, botId: false });
  ...
  const loadPendingPairings = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        // Filter for Mezon platform only
  ...
  // Test Mezon connection
  const handleTestConnection = async () => {
    // Mark fields as touched to show validation errors
    setTouched({ token: true, botId: true });

    if (!mezonToken.trim() || !botId.trim()) {
    ...
    try {
      const result = await channel.testPlugin.invoke({
        ...
        token: mezonToken.trim(),
        extraConfig: {
          appId: botId.trim(), // Reuse appId field for botId
        },
      });
  ...
  const handleAutoEnable = async () => {
    try {
      console.log('[MezonConfig] Auto-enabling plugin with credentials...');
      ...
      console.log('[MezonConfig] enablePlugin result:', result);

      if (result.success) {
        Message.success(t('settings.mezon.pluginEnabled', 'Mezon bot enabled'));
        const statusResult = await channel.getPluginStatus.invoke();
        console.log('[MezonConfig] getPluginStatus result:', statusResult);
        if (statusResult.success && statusResult.data) {
          const mezonPlugin = statusResult.data.find((p) => p.type === 'mezon');
          console.log('[MezonConfig] Mezon plugin status:', mezonPlugin);
          onStatusChange(mezonPlugin || null);
        }
      } else {
        console.error('[MezonConfig] enablePlugin failed:', result.msg);
        Message.error(result.msg || t('settings.mezon.enableFailed', 'Failed to enable Mezon plugin'));
      }
    } catch (error: any) {
      console.error('[MezonConfig] Auto-enable failed:', error);
      Message.error(error.message || t('settings.mezon.enableFailed', 'Failed to enable Mezon plugin'));
    }
  };
  ...
  const handleModelSelect = async (provider: IProvider, modelName: string) => {
    ...
    try {
      await ConfigStorage.set('assistant.mezon.defaultModel', {
        id: provider.id,
        useModel: modelName,
      });
      Message.success(t('settings.assistant.modelSaved', 'Model saved'));
    } catch (error) {
      console.error('[MezonConfig] Failed to save model:', error);
  ...
  return (
    <div className='flex flex-col gap-24px'>
      {/* Bot Token */}
      <PreferenceRow label={t('settings.mezon.botToken', 'Bot Token')} description={t('settings.mezon.botTokenDesc', 'Token from Mezon Developer Portal for your bot')} required>
        <Input.Password
          value={mezonToken}
          onChange={(value) => {
            setMezonToken(value);
          ...
          placeholder={hasExistingUsers || pluginStatus?.hasToken ? '••••••••••••••••' : 'Enter bot token'}
          style={{ width: 240 }}
          status={touched.token && !mezonToken.trim() && !pluginStatus?.hasToken ? 'error' : undefined}
          visibilityToggle
      ...
      {/* Bot ID */}
      <PreferenceRow label={t('settings.mezon.botId', 'Bot ID')} description={t('settings.mezon.botIdDesc', 'Bot ID from Mezon Developer Portal (used for authentication)')} required>
        <Input
          value={botId}
          onChange={(value) => {
            setBotId(value);
          ...
          placeholder={hasExistingUsers || pluginStatus?.hasToken ? '••••••••••••••••' : 'Enter bot ID'}
          style={{ width: 240 }}
      ...
      {!hasExistingUsers && !pluginStatus?.connected && (
        <div className='flex justify-end'>
          {pluginStatus?.hasToken && !mezonToken.trim() && !botId.trim() ? (
            <span className='text-12px text-t-tertiary mr-12px self-center'>{t('settings.mezon.credentialsSaved', 'Credentials already configured. Enter new values to update.')}</span>
          ) : null}
          <Button type='primary' loading={testLoading} onClick={handleTestConnection} disabled={pluginStatus?.hasToken && !mezonToken.trim() && !botId.trim()}>
            {t('settings.mezon.testAndConnect', 'Test & Connect')}
          </Button>
        </div>
      )}

      {/* Default Model Selection */}
      <PreferenceRow label={t('settings.assistant.defaultModel', 'Default Model')} description={t('settings.mezon.defaultModelDesc', 'Model used for Mezon conversations')}>
        <Dropdown
          trigger='click'
          position='br'
          droplist={
      ...
      {pluginStatus?.enabled && authorizedUsers.length === 0 && (
        <div className={`rd-12px p-16px border ${pluginStatus?.connected ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' : pluginStatus?.error ? 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800' : 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800'}`}>
          <SectionHeader title={t('settings.mezon.connectionStatus', 'Connection Status')} action={<span className={`text-12px px-8px py-2px rd-4px ${pluginStatus?.connected ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : pluginStatus?.error ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300'}`}>{pluginStatus?.connected ? t('settings.mezon.statusConnected', 'Connected') : pluginStatus?.error ? t('settings.mezon.statusError', 'Error') : t('settings.mezon.statusConnecting', 'Connecting...')}</span>} />
          ...
          {pluginStatus?.connected && (
            <div className='text-14px text-t-secondary space-y-8px'>
              ...
              <p className='m-0'>
                <strong>4.</strong> {t('settings.mezon.step4', 'Once approved, you can start chatting with the AI assistant through Mezon!')}
              </p>
            </div>
          )}
          {!pluginStatus?.connected && !pluginStatus?.error && <div className='text-14px text-t-secondary'>{t('settings.mezon.waitingConnection', 'WebSocket connection is being established. Please wait...')}</div>}
        </div>
      )}
              ...
              {authorizedUsers.map((user) => (
                <div key={user.id} className='flex items-center justify-between bg-fill-2 rd-8px p-12px'>
                  <div className='flex-1'>
                    ...
                    <div className='text-12px text-t-tertiary mt-4px'>
                      {t('settings.assistant.platform', 'Platform')}: Mezon
                      <span className='mx-8px'>|</span>
