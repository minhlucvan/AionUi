        ...
        if (block.type === 'thinking' && block.thinking) {
          return (
            <Collapse key={i} bordered={false}>
              ...
              >
                <pre className='text-12px text-t-secondary font-mono bg-bg-3 p-8px rd-4px overflow-auto whitespace-pre-wrap break-words' style={{ maxHeight }}>
                  {block.thinking}
                </pre>
              </Collapse.Item>
            </Collapse>
            ...
            <div key={i} className='border-1px border-solid border-border-2 rd-6px overflow-hidden'>
              ...
              <Collapse bordered={false}>
                <Collapse.Item name='input' header={<span className='text-12px text-t-tertiary'>Input</span>}>
                  <pre className='text-12px text-t-secondary font-mono bg-bg-3 p-8px rd-4px overflow-auto whitespace-pre-wrap break-all' style={{ maxHeight }}>
                    {inputStr}
                  </pre>
                </Collapse.Item>
              </Collapse>
        ...
        if (block.type === 'tool_result') {
          ...
          return (
            <div key={i} className='border-1px border-solid border-border-2 rd-6px overflow-hidden'>
              ...
              <pre className='text-12px text-t-secondary font-mono p-8px overflow-auto whitespace-pre-wrap break-all' style={{ maxHeight }}>
                {resultContent}
              </pre>
            </div>
          );
    ...
    <div className='mt-4px space-y-4px'>
      {results.map((result) => (
        <Collapse key={result.toolUseId} bordered={false}>
          ...
          >
            <pre className='text-12px text-t-secondary font-mono bg-bg-3 p-8px rd-4px overflow-auto max-h-200px whitespace-pre-wrap break-all'>{result.content || '(empty)'}</pre>
          </Collapse.Item>
        </Collapse>
      ))}
    </div>
...
const MessageCard: React.FC<{ message: ParsedMessage; index: number }> = ({ message, index }) => {
  ...
  const label = isUser ? 'User' : isAssistant ? 'Assistant' : isSystem ? 'System' : isMeta ? 'Meta' : isToolResult ? 'Tool Result' : isSummary ? 'Summary' : message.type;

  const usageText = message.usage ? `${formatTokens(message.usage.input_tokens)} in / ${formatTokens(message.usage.output_tokens)} out` : null;
  ...
  return (
    <div className='mb-8px'>
      <div className='flex items-center gap-8px mb-2px'>
        ...
        {message.model && <span className='text-10px text-t-quaternary font-mono'>{message.model}</span>}
        {usageText && <span className='text-10px text-t-quaternary'>{usageText}</span>}
      </div>
      <div className='ml-11px pl-10px border-l-1px border-l-solid border-l-border-2'>
        {/* Content */}
        {typeof message.content === 'string' ? <div className='text-13px text-t-secondary whitespace-pre-wrap break-words max-h-400px overflow-auto'>{message.content || (isToolResult ? '' : '(empty)')}</div> : <ContentBlockRenderer blocks={message.content as ContentBlock[]} />}
  ...
  return (
    <div className='h-full flex flex-col'>
      ...
      <div className='flex items-center gap-8px px-12px py-8px border-b-1px border-b-solid border-b-border-2 flex-shrink-0'>
        <Input.Search placeholder='Search messages...' size='small' style={{ width: 200 }} value={filter} onChange={setFilter} allowClear />
        <div className='flex gap-4px'>
          <Tag size='small' color={typeFilter === 'user' ? 'blue' : undefined} className='cursor-pointer' onClick={() => handleTypeFilter('user')}>
            User ({typeCounts.user})
          </Tag>
          <Tag size='small' color={typeFilter === 'assistant' ? 'cyan' : undefined} className='cursor-pointer' onClick={() => handleTypeFilter('assistant')}>
            Assistant ({typeCounts.assistant})
          </Tag>
          <Tag size='small' color={typeFilter === 'tool' ? 'purple' : undefined} className='cursor-pointer' onClick={() => handleTypeFilter('tool')}>
            Tools ({typeCounts.tool})
          </Tag>
          <Tag size='small' color={typeFilter === 'system' ? 'gray' : undefined} className='cursor-pointer' onClick={() => handleTypeFilter('system')}>
