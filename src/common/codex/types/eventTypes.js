...
exports.CodexAgentEventType = void 0;
// Codex Agent Event Types
var CodexAgentEventType;
(function (CodexAgentEventType) {
     ...
     * }
     * */
    CodexAgentEventType["SESSION_CONFIGURED"] = "session_configured";
    /**
     * 任务开始事件 - 代理已开始任务
     * prompt: '你好 codex'
     * payload: { model_context_window: number | null }
     */
    CodexAgentEventType["TASK_STARTED"] = "task_started";
    /**
     * 任务完成事件 - 代理已完成所有操作
     * prompt: '你好 codex'
     * payload: { last_agent_message: string | null }
     */
    CodexAgentEventType["TASK_COMPLETE"] = "task_complete";
     ...
     */
    CodexAgentEventType["AGENT_MESSAGE_DELTA"] = "agent_message_delta";
    /**
     * 代理消息事件 - 代理文本输出消息（完整输出消息）
     * prompt: '你好 codex'
     * payload: { message: string }
     */
    CodexAgentEventType["AGENT_MESSAGE"] = "agent_message";
    /**
     * 用户消息事件 - 用户/系统输入消息（发送给模型的内容）
     * prompt: '你好 codex'
     * payload: { message: string, kind: InputMessageKind | null, images: string[] | null }
     */
    CodexAgentEventType["USER_MESSAGE"] = "user_message";
    /**
     * 代理推理事件 - 来自代理的推理事件（完整的思考文本）
     * prompt: '我想翻阅codex收发消息原始数据格式的接口文档,应该去哪获取？'
     * payload: { text: string }
     */
    CodexAgentEventType["AGENT_REASONING"] = "agent_reasoning";
    /**
     * 代理推理增量事件 - 来自代理的推理增量事件（流式增量输出文本）
     * prompt: '可以给我一个openAI 官方 url吗？'
     * payload: { delta: string }
     */
    CodexAgentEventType["AGENT_REASONING_DELTA"] = "agent_reasoning_delta";
    /**
     * 代理推理原始内容事件 - 来自代理的原始思维链
     * prompt: 'TypeScript 编译错误：'Property X does not exist'，请帮我分析原因'
     * payload: { text: string }
     */
    CodexAgentEventType["AGENT_REASONING_RAW_CONTENT"] = "agent_reasoning_raw_content";
    /**
     * 代理推理原始内容增量事件 - 来自代理的推理内容增量事件
     * payload: { delta: string }
     */
    CodexAgentEventType["AGENT_REASONING_RAW_CONTENT_DELTA"] = "agent_reasoning_raw_content_delta";
    /**
     * 代理推理章节分隔事件 - 当模型开始新的推理摘要部分时发出信号（例如，新的标题块）
     * prompt: 'TypeScript 编译错误：'Property X does not exist'，请帮我分析原因'
     * payload: {}
     */
    CodexAgentEventType["AGENT_REASONING_SECTION_BREAK"] = "agent_reasoning_section_break";
    // Usage / telemetry
    /**
     ...
     */
    CodexAgentEventType["TOKEN_COUNT"] = "token_count";
     ...
     */
    CodexAgentEventType["EXEC_COMMAND_BEGIN"] = "exec_command_begin";
    /**
     ...
     */
    CodexAgentEventType["EXEC_COMMAND_OUTPUT_DELTA"] = "exec_command_output_delta";
    /**
     * 执行命令结束事件 - 表示命令执行完成
     * prompt: 'TypeScript 编译错误：'Property X does not exist'，请帮我分析原因'
     * payload: {
     ...
     * }
     */
    CodexAgentEventType["EXEC_COMMAND_END"] = "exec_command_end";
    /**
     * 执行批准请求事件 - 请求批准命令执行
     ...
     */
    CodexAgentEventType["EXEC_APPROVAL_REQUEST"] = "exec_approval_request";
     ...
     */
    CodexAgentEventType["APPLY_PATCH_APPROVAL_REQUEST"] = "apply_patch_approval_request";
    /**
     ...
     */
    CodexAgentEventType["PATCH_APPLY_BEGIN"] = "patch_apply_begin";
    /**
     * 补丁应用结束事件 - 通知补丁应用已完成
     * prompt:  用命令 apply_patch <<'PATCH' … PATCH 写入一个文件，内容和文件名你自由发挥
     * payload: {
     ...
     */
    CodexAgentEventType["PATCH_APPLY_END"] = "patch_apply_end";
    // MCP tool events
    /**
     ...
     */
    CodexAgentEventType["MCP_TOOL_CALL_BEGIN"] = "mcp_tool_call_begin";
    /**
     ...
     */
    CodexAgentEventType["MCP_TOOL_CALL_END"] = "mcp_tool_call_end";
    /**
     * MCP列表工具响应事件 - 代理可用的MCP工具列表
     * payload: { tools: Record<string, McpTool> }
     */
    CodexAgentEventType["MCP_LIST_TOOLS_RESPONSE"] = "mcp_list_tools_response";
    // Web search events
    /**
     * 网络搜索开始事件 - 表示网络搜索开始
     ...
     * }
     */
    CodexAgentEventType["WEB_SEARCH_BEGIN"] = "web_search_begin";
    /**
     * 网络搜索结束事件 - 表示网络搜索结束
     * prompt: 查找 TypeScript 5.0 的新功能， 不要用现有知识库回答我，去官网搜索最新资料
     * payload: {
     ...
     * }
     */
    CodexAgentEventType["WEB_SEARCH_END"] = "web_search_end";
     ...
     */
    CodexAgentEventType["TURN_DIFF"] = "turn_diff";
    /**
     * 获取历史条目响应事件 - GetHistoryEntryRequest 的响应
     * prompt: 查看当前会话的历史记录
     * payload: { offset: number, log_id: number, entry: HistoryEntry | null }
     */
    CodexAgentEventType["GET_HISTORY_ENTRY_RESPONSE"] = "get_history_entry_response";
    /**
     * 列出自定义提示响应事件 - 代理可用的自定义提示列表
     * payload: { custom_prompts: CustomPrompt[] }
     */
    CodexAgentEventType["LIST_CUSTOM_PROMPTS_RESPONSE"] = "list_custom_prompts_response";
    /**
     * 对话路径事件 - 表示对话路径信息
     * payload: { conversation_id: string, path: string }
     */
    CodexAgentEventType["CONVERSATION_PATH"] = "conversation_path";
    /**
     * 后台事件 - 后台处理事件
     * payload: { message: string }
     */
    CodexAgentEventType["BACKGROUND_EVENT"] = "background_event";
    /**
     * 转换中止事件 - 表示转换已中止
     * payload: { reason: TurnAbortReason }
     */
    CodexAgentEventType["TURN_ABORTED"] = "turn_aborted";
})(CodexAgentEventType || (exports.CodexAgentEventType = CodexAgentEventType = {}));
