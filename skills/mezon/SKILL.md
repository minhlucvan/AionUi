...
npx ts-node skills/mezon/cli.ts status

# List channels with activity
npx ts-node skills/mezon/cli.ts list-channels

# Read last 20 messages from a channel
npx ts-node skills/mezon/cli.ts read-messages --channel-id abc123 --limit 20

# Read from a thread
npx ts-node skills/mezon/cli.ts read-messages --channel-id abc123 --thread-id thread456

# Send a message
npx ts-node skills/mezon/cli.ts send-message \
...
# Search messages
npx ts-node skills/mezon/cli.ts search --query "deployment" --limit 10

# Get channel summary (participants, time range, all messages)
npx ts-node skills/mezon/cli.ts summary --channel-id abc123

# Stream messages in real-time (long-running)
npx ts-node skills/mezon/cli.ts listen

# Start as MCP stdio server
npx ts-node skills/mezon/cli.ts serve
```
...
const result = JSON.parse(
  execSync('npx ts-node skills/mezon/cli.ts read-messages --channel-id abc123', {
...
// Send a message
execSync(
  'npx ts-node skills/mezon/cli.ts send-message --channel-id abc123 --clan-id clan456 --text "Summary: ..."',
  { env: { ...process.env, MEZON_BOT_TOKEN: 'xxx', MEZON_BOT_ID: 'yyy' } }
);
```
...
claude mcp add mezon -- npx ts-node skills/mezon/cli.ts serve
```
...
```json
{
  ...
  "transport": {
    "type": "stdio",
    "command": "npx",
    "args": ["ts-node", "skills/mezon/cli.ts", "serve"],
    "env": {
...
1. Add the command function in `skills/mezon/cli.ts`
